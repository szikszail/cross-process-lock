...
describe("metadata", () => {
    ...
    describe("readMetadata", () => {
        test("should handle file error", async () => {
            ...
            expect(readFile).toHaveBeenCalledWith(FILE, { encoding: "utf8" }, expect.any(Function));
        });
    ...
    describe("saveMetadata", () => {
        ...
        test("should handle file error", async () => {
            (writeFile as unknown as jest.Mock).mockImplementation((_1, _2, _3, fn) => {
                fn(new Error("writeFile Error"));
            });

            await expect(saveMetadata(FILE, LOCK_TIME)).rejects.toThrow("writeFile Error");

            const METADATA = JSON.stringify({ pID: process.pid, lockTime: LOCK_TIME });
            expect(writeFile).toHaveBeenCalledWith(FILE, METADATA, { encoding: "utf8" }, expect.any(Function));
        });
